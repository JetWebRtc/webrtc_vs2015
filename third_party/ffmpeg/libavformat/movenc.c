...
static int mov_write_chpl_tag(AVIOContext *pb, AVFormatContext *s)
{
    ...
    for (i = 0; i < nb_chapters; i++)
    {
        ...
        AVDictionaryEntry *t;
        avio_wb64(pb, av_rescale_q(c->start, c->time_base, (AVRational)
        {
            1,10000000
        }));
...
static int mov_create_chapter_track(AVFormatContext *s, int tracknum)
{
    ...
    for (i = 0; i < s->nb_chapters; i++)
    {
        ...
        int64_t end = av_rescale_q(c->end, c->time_base, (AVRational)
        {
            1,MOV_TIMESCALE
        });
        pkt.pts = pkt.dts = av_rescale_q(c->start, c->time_base, (AVRational)
        {
            1,MOV_TIMESCALE
        });
